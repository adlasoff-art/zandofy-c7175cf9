...
export function VendorProductManager({ storeId }: { storeId: string }) {
  ...
  const loadProducts = useCallback(async () => {
    ...
    const { data } = await supabase
      .from("products")
      .select("id, name, name_fr, price, original_price, currency, description, short_description, moq, sku, is_new, is_sale, discount, material, origin_country, category_id, trend_tag_id, store_id, promo_start_date, promo_end_date, flash_timer_enabled, weight_grams, length_cm, width_cm, height_cm, publish_status")
      .eq("store_id", storeId)
