...
export function AdminUserPicker({
  ...
  const { data: hits = [], isFetching } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await sb.rpc("admin_search_users", {
        p_query: debounced,
        p_limit: 20,
      });
  ...
  const filteredHits = useMemo(
    () => (excludeUserId ? hits.filter((u) => u.id !== excludeUserId) : hits),
    [hits, excludeUserId],
  );

  const buttonLabel = useMemo(() => {
    if (!value) return placeholder ?? "Choisir un utilisateur";
        ...
        <PopoverContent className="w-[340px] p-2" align="start">
          <div className="relative">
            ...
            <Input
              ...
              placeholder="Email, prénom ou nom (min. 2 car.)"
              className="pl-7 h-9"
            />
          </div>
          ...
          <div className="mt-2 max-h-64 overflow-y-auto">
            {debounced.length < 2 ? (
              ...
              <p className="text-xs text-muted-foreground px-2 py-3 text-center">Aucun utilisateur trouvé.</p>
            ) : (
