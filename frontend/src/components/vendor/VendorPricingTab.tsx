  ...
  const { data: productOverride } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await (supabase as any)
        ...
        .eq("store_id", storeId)
        .maybeSingle();
      return data?.max_products_override ?? null;
    },
  });
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Choisissez le package adapté à votre activité ou souscrivez à des services individuels.
        </p>
      </div>

      {/* Current tier & product limit */}
      <div className="bg-card border border-border rounded-lg p-4">
        <div className="flex items-center gap-2 mb-2">
          <ShoppingBag size={18} className="text-primary" />
          <h3 className="text-sm font-bold text-foreground">Votre niveau actuel</h3>
        </div>
        <div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
          <Badge className={tierConfig.badgeClass}>{tierConfig.label}</Badge>
          <span>
            Limite produits : <span className="font-bold text-foreground">
              {effectiveMaxProducts >= 1999 ? "Illimité" : effectiveMaxProducts}
            </span>
            {productOverride != null && (
              <span className="text-[10px] text-primary ml-1">(personnalisé par l'admin)</span>
            )}
          </span>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {packages.map((pkg: any) => {
            ...
            const linkedTier = PACKAGE_TIER_MAP[pkg.slug];
            const linkedTierConfig = linkedTier ? VENDOR_TIERS[linkedTier] : null;
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex items-center gap-1">
                    {linkedTierConfig && (
                      <Badge variant="outline" className={`text-[10px] ${linkedTierConfig.badgeClass}`}>
                        {linkedTierConfig.label} · {linkedTierConfig.maxProducts >= 1999 ? "∞" : linkedTierConfig.maxProducts} produits
                      </Badge>
                    )}
                ...
                <ul className="mt-3 space-y-1.5 flex-1 text-xs text-muted-foreground">
                  ...
                  <li className="flex items-center gap-2">
                    ...
                    Hub: {pkg.hub_storage_free_kg > 0 ? `${pkg.hub_storage_free_kg} kg gratuit` : "Payant (0,25$/jour/kg)"}
                  </li>
                  <li className="flex items-center gap-2">
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mb-3">
          Souscrivez à des services séparément sans changer de package. Les services déjà inclus dans votre package sont marqués.
        </p>
        <VendorIndividualServices storeId={storeId} billingCycle={billingCycle} kycVerified={kycVerified ?? false} />
      </div>
