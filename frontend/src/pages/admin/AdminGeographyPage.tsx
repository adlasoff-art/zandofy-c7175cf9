...
// Types
interface CityRow { id: string; name: string; country_code: string; latitude: number; longitude: number; population: number | null; }
...
function CitiesTab() {
  ...
  const [form, setForm] = useState({ name: "", country_code: "CD", latitude: "", longitude: "", population: "" });
  const [editId, setEditId] = useState<string | null>(null);
  const [showForm, setShowForm] = useState(false);

  const fetch = useCallback(async () => {
    setLoading(true);
    let q = supabase.from("cities").select("id, name, country_code, latitude, longitude, population").order("name").limit(500);
  ...
  const handleSave = async () => {
    ...
    setShowForm(false);
    setEditId(null);
    setForm({ name: "", country_code: filterCountry || "CD", latitude: "", longitude: "", population: "" });
    fetch();
    toast.success(editId ? "Ville modifiée" : "Ville ajoutée");
  };

  const handleEdit = (c: CityRow) => {
    setForm({ name: c.name, country_code: c.country_code, latitude: String(c.latitude), longitude: String(c.longitude), population: c.population ? String(c.population) : "" });
    setEditId(c.id);
    setShowForm(true);
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <Button size="sm" onClick={() => { setShowForm(true); setEditId(null); setForm({ name: "", country_code: filterCountry || "CD", latitude: "", longitude: "", population: "" }); }}>
      ...
      {showForm && (
        <div className="bg-card border border-border rounded-lg p-4 space-y-3">
          ...
          <div className="w-40">
            ...
            <Input className="mt-1" type="number" value={form.population} onChange={e => setForm(f => ({ ...f, population: e.target.value }))} />
          </div>
          ...
          <TableBody>
            {cities.map(c => (
              <TableRow key={c.id}>
                ...
                <TableCell>{getCountryName(c.country_code)} ({c.country_code})</TableCell>
            ...
            {cities.length === 0 && <TableRow><TableCell colSpan={4} className="text-center text-muted-foreground py-8">Aucune ville</TableCell></TableRow>}
          </TableBody>
        </Table>
      )}
    </div>
...
function CommunesTab() {
  ...
  // Load distinct cities for filter
  useEffect(() => {
    supabase.from("cities").select("name").eq("country_code", filterCountry).order("name").then(({ data }) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        <div className="w-48">
          <CountryCombobox value={filterCountry} onChange={v => { setFilterCountry(v); setFilterCity(""); }} label="Pays" showNone={false} />
        </div>
        <div className="w-48">
          <Label className="text-xs text-muted-foreground">Ville</Label>
          <select className="mt-1 w-full px-3 py-2 text-sm border border-border rounded-md bg-card" value={filterCity} onChange={e => setFilterCity(e.target.value)}>
        ...
        <div className="bg-card border border-border rounded-lg p-4 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              ...
              <select className="mt-1 w-full px-3 py-2 text-sm border border-border rounded-md bg-card" value={form.city} onChange={e => setForm(f => ({ ...f, city: e.target.value }))}>
                <option value="">—</option>
                {cities.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
...
function QuartiersTab() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 flex-wrap">
        <div className="w-44">
          <CountryCombobox value={filterCountry} onChange={v => { setFilterCountry(v); setFilterCity(""); setFilterCommune(""); }} label="Pays" showNone={false} />
        </div>
        <div className="w-40">
          <Label className="text-xs text-muted-foreground">Ville</Label>
          <select className="mt-1 w-full px-3 py-2 text-sm border border-border rounded-md bg-card" value={filterCity} onChange={e => { setFilterCity(e.target.value); setFilterCommune(""); }}>
            <option value="">Toutes</option>
            {cities.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
        </div>
        <div className="w-44">
          ...
          <select className="mt-1 w-full px-3 py-2 text-sm border border-border rounded-md bg-card" value={filterCommune} onChange={e => setFilterCommune(e.target.value)}>
            <option value="">Toutes</option>
        ...
        <div className="bg-card border border-border rounded-lg p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select className="mt-1 w-full px-3 py-2 text-sm border border-border rounded-md bg-card" value={form.commune_id} onChange={e => setForm(f => ({ ...f, commune_id: e.target.value }))}>
                <option value="">—</option>
                {communes.map(c => <option key={c.id} value={c.id}>{c.name} ({c.city})</option>)}
              </select>
            </div>
