-- ============================================================
-- Filtrage strict des transitaires au checkout (preview env)
-- Mêmes règles que la migration prod 20260428190700_strict_forwarder_eligibility.sql
-- ============================================================
...
AS $$
  ...
  SELECT
    f.id, f.name, f.slug, f.logo_url, f.is_platform_owned, f.supported_modes,
    EXISTS (
      SELECT 1
    ...
    fpp.id, fpp.service_class, fpp.transit_min_days, fpp.transit_max_days
...
RETURNS TABLE (
  forwarder_id uuid, forwarder_name text, forwarder_slug text, logo_url text,
  tier text, mode text, price_multiplier numeric,
  transit_min_days integer, transit_max_days integer,
  is_platform_owned boolean, has_profile_for_zone boolean, unavailable_message text
)
LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path TO 'public'
AS $$
BEGIN
  RETURN QUERY
  SELECT f.id, f.name, f.slug, f.logo_url,
    COALESCE(fpp.service_class, 'standard')::text,
    fpp.mode::text, 1::numeric,
    fpp.transit_min_days, fpp.transit_max_days,
    f.is_platform_owned, true, f.unavailable_message
  FROM public.forwarders f
