-- Purpose: Ensure orders.shipping_mode exists and get_customer_tracking is safe.
-- Tables: public.orders, function public.get_customer_tracking
-- Rollback: none needed (additive column; RPC REPLACE only)
-- Risk: low — IF NOT EXISTS; RPC already used in staging/prod when column present

ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS shipping_mode text;

COMMENT ON COLUMN public.orders.shipping_mode IS
  'Delivery mode hint for customer tracking (air/sea/road/local/etc).';

-- Re-apply RPC so SELECT never references a missing column after partial deploys
...
COMMENT ON FUNCTION public.get_customer_tracking(uuid)
  IS 'Returns consolidated tracking info for an order owned by the authenticated customer.';
