-- Purpose: Allow city_id in sanitized discovery_prefs (I7 true-city matching).
-- Tables: profiles via set_own_discovery_prefs
-- Rollback: restore body from 20260920203000_set_own_discovery_prefs_sanitize.sql
-- Risk: low — additive field; invalid UUIDs dropped
...
COMMENT ON FUNCTION public.set_own_discovery_prefs(jsonb) IS
  'Authenticated user writes own sanitized profiles.discovery_prefs (incl. city_id); soft-fills empty gender.';
